...
class Level:
    def __init__(self, grass_patch_model, grass_plane_model, plane_scale=2, grass_scale=1, grass_amount=10):
    ...
    def generate_grass(self, grass_patch_model):
        for i in range(self.grass_amount):
            ...
            random_coordinates = random.sample(range(0, self.map_size), 3)
            new_grass = Object(Vector(0, 0, 0), Vector(0, 0, 0),
                               Vector(random_coordinates[0], random_coordinates[1], random_coordinates[2]),
                               grass_patch_model)
